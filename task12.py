...
pg.mixer.music.load(r"audio\soundsbackgroundogg.wav")
pg.mixer.music.play(-1)

crash_sound = pg.mixer.Sound(r"audio\800655__isaac1708__car-accident.wav")
...
while flag_play:
    ...
    if not game_over:
        ...
        if not crash_played:
            pg.mixer.music.stop()
            crash_sound.play()
            length = crash_sound.get_length()
            pg.time.wait(int(length * 1000))
            crash_played = True
